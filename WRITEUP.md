...
This document provides steps which were taken when the projects was implemented alongside the rationales for the steps. 

At a high level 3 models were tested for the application. Both SSD Mobile Net V1 & SSD Mobile Net V2 made inferences with good enough accuracy & latency. SSD Resnet 50 v1 FPN 640x640 had slightly better accuracy but very high latency. 
...
2. Replace sub-graph in the model optimiser. [refer here](https://docs.openvinotoolkit.org/2019_R3/_docs_MO_DG_prepare_model_customize_model_optimizer_Subgraph_Replacement_Model_Optimizer.html)
...
During the implementation 3rd option was used. The absolute path for C++ library was given as an input argument to the app and it was added as an extension in the ```Network class```.
...
|Tensorflow model out | IR model out |
|---|---|
| <img src="images/tensorflow_model.png" width="400"/>  |  <img src="images/ssd_mobil_with_inference_.png" width="400"/>  |


note: ```tensorflow_test.py``` is added as a separate file in the workspace.
## Assess Model Use Cases

There can be many potential use cases of such a people counting app.
1. It can be used as a method of crowd control in a confined place. This is useful to enforce social distancing during COVID-19 pandemic.
2. It can be used as a means of surveillance to check for un-authorised access. 
...
- Model speed  
    Model speed refers to the inference time taken on each input. To speed up the inference it might be necessary to sacrifice some of the accuracy. This should be discussed with the end user.
- Model size   
    Bigger models again tends to be resource heavy & might not fit in to a resource constrained environment.
- Lighting condition  
    This will impact the accuracy of the application if the model has not been trained on different kinds of lighting conditions. The end user needs to specify how the app would be used & a special model training might be necessary if the model has not been trained for the specified scenario.
...
### Model summary
|Model Name | latency | accuracy | Total time for inference |
|---|---| ---| ---|
| SSD Resnet 50 v1 FPN 640x640  | 2581ms  | 1076 | 1 hour|
| SSD Mobile Net V1  |  42ms | 870 | 1 minute |
| SSD Mobile Net V2  |  67ms | 996 | 1.5 minutes|
...
- Model 1: [SSD Resnet 50 v1 FPN 640x640]
  ...
  - Command & arguments used to convert the model to IR
    ...
    ```
  - The model was insufficient for the app because it took a long time to do the inference. The average inference time for this was 2581.  It was able to recognise a person in 1076 frames.
